...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void _showLanguageSelectionDialog(BuildContext context) {
    showDialog(
      ...
      builder: (context) {
        return WillPopScope(
          onWillPop: () async {
            return false; // Prevent closing the dialog with the back button
          },
          child: AlertDialog(
            shape: RoundedRectangleBorder(
              borderRadius: BorderRadius.circular(20.0),
            ),
            backgroundColor: Color(0xFF1F2A44), // Dark blue background for the dialog
            title: Text(
              'Select Language',
              style: TextStyle(
                color: Colors.white,
            ...
            content: Column(
              ...
              children: [
                ...
                SizedBox(height: 20),
                Divider(color: Colors.white54), // Divider for better separation
                _buildLanguageButton(
                  context,
                  label: 'Cancel',
                  icon: Icons.cancel,
  ...
  // Helper method to build buttons with icons and styling
...
class _QuizScreenState extends State<QuizScreen> {
  ...
  int _correctCount = 0;
  int _wrongCount = 0;
  bool _quizCompleted = false; // New flag to check if the quiz is completed
  ...
  void _answerQuestion(String answer) {
    if (_quizCompleted) return; // Prevent answering if the quiz is completed
  ...
  void _showEndModal(bool isTimeUp) {
    ...
    showDialog(
      context: context,
      barrierDismissible: false, // Prevent closing the modal when tapping outside
      barrierColor: Colors.black54, // Optional: add a dim background effect
      builder: (BuildContext context) {
        return WillPopScope(
          onWillPop: () async {
            return false; // Prevent closing the modal with the back button
          },
          child: AlertDialog(
